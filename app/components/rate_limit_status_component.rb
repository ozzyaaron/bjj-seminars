class RateLimitStatusComponent < ApplicationComponent
  include RateLimitHelper

  def view_template
    return unless user_signed_in? && should_show_status?

    div(class: "bg-yellow-50 border border-yellow-200 rounded-md p-4 mb-6") do
      div(class: "flex") do
        warning_icon
        div(class: "ml-3") do
          h3(class: "text-sm font-medium text-yellow-800") { "Rate Limit Status" }
          div(class: "mt-2 text-sm text-yellow-700") do
            p { rate_limit_warning_message }
          end
        end
      end
    end
  end

  private

  def should_show_status?
    info = rate_limit_info_for_user
    info && info[:remaining_seminars] <= 10
  end

  def warning_icon
    div(class: "flex-shrink-0") do
      svg(
        class: "h-5 w-5 text-yellow-400",
        viewBox: "0 0 20 20",
        fill: "currentColor"
      ) do |s|
        s.path(
          fill_rule: "evenodd",
          d: "M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z",
          clip_rule: "evenodd"
        )
      end
    end
  end
end